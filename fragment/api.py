...
class FragmentAPI:
  ...
  def _setup_client_data(self):
    try:
      ...
      raise Exception('Error setting Fragment client data.')
  
  def _send_fragment_request(self, method: FragmentMethod, body):
    try:
      ...
      print(f'Unexpected error in _send_fragment_request: {error}')
  ...
  def _getBuyStarsLink(self, account, req_id):
    try:
      ...
      print(f'Unexpected error in _getBuyStarsLink: {error}')
      return None
  ...
  def create_stars_transaction(self, recipient_id, quantity, wallet_account):
    try:
      ...
      if init_err := init_buy.get('error'):
        return self._error(f'Error initializing buy request: {init_err}')
      ...
      if buy_err := buy_transaction.get('error'):
        return self._error(f'Error getting buy transaction: {buy_transaction_error}')
